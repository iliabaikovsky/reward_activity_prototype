...
Справа от рамки телефона — **glass rail** (`LifecycleSimulatorPanel`, данные `LIFECYCLE_STEPS` в `src/rewardLifecycle/lifecycleSteps.ts`). **10 шагов**, старт с **шага 1** — пустое состояние (§0); стрелки **← / →** переключают сценарий §12. На каждом шаге обновляются: **Available rewards**, второй кошелёк, **Lifetime cashback**, **Upcoming**, превью **Activity feed** на главном экране и полная лента на экране **Activity feed** (с учётом фильтров).

На широком экране под rail — раскрываемая **Справка** (ссылка на этот файл + `docRef` текущего шага). Отдельного select и debug-снимка состояния нет — переключение только стрелками.
